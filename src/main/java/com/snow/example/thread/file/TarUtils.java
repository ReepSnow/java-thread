...
public abstract class TarUtils {
    ...
    private static void dearchive(File destFile, TarArchiveInputStream tais)
        ...
        while ((entry = tais.getNextTarEntry()) != null) {
            ...
            // 文件
            String fileNameEntryTemp =entry.getName();
            String fileNameTar=null;
            if(File.separator.equals("\\")&&fileNameEntryTemp.contains("/")){
                fileNameTar = fileNameEntryTemp.replace("/","\\");
                fileNameTar=fileNameTar.substring(fileNameTar.indexOf("\\")+1);
            }else if(File.separator.equals("/")&&fileNameEntryTemp.contains("/")){
                fileNameTar=fileNameEntryTemp.substring(fileNameEntryTemp.indexOf("/")+1);
            }else {
                fileNameTar=fileNameEntryTemp;
            }
            String dir = destFile.getPath() + File.separator +fileNameTar;
    ...
    public static void fileProberHsh(File dirFile) throws Exception {
